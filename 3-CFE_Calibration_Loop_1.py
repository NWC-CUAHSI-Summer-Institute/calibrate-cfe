...
class Spotpy_setup(object):
    ...
    def simulation(self, vector):
        ...
        spinup_start_idx_nldas = np.where(
            self.df_forcing['date'] == self.time_split['spinup-for-calibration']['start_datetime'])
        spinup_end_idx_nldas = np.where(
            self.df_forcing['date'] == self.time_split['spinup-for-calibration']['end_datetime'])
        ...
        if self.print_all_process:
            print(
                f'###----------output length: {len(self.output_lists["land_surface_water__runoff_depth"])}.---------###')
